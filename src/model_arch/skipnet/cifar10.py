...
class ResNetRecurrentGateSP(nn.Module):
    ...
    def forward(self, x, device):
        ...
        self.control.hidden = self.control.init_hidden(batch_size, device)
...
class RNNGatePolicy(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            proj = self.proj(out.squeeze())
            prob = self.prob(proj).reshape([-1, 1])
            bi_prob = torch.cat([1 - prob, prob], dim=1)
        ...
        for g in range(3):
            for i in range(0 + int(g == 0), self.num_layers[g]):
                ...
                if mask.sum() == 1:
                    ...
                    prev = x = (1 - mask).expand_as(prev) * prev
